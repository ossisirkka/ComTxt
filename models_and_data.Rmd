...
get_data_for_spesific_mca <- function(active_df, sup_df, karsittavat_str = index_str){
  ...
  observations_with_more_than_20p_missing_values <- apply(dat_with_na_levels, 1, function(x) sum(is.na(x)) < 0.2 * ncol(dat_with_na_levels))
  dat_with_na_levels <- dat_with_na_levels[observations_with_more_than_20p_missing_values,]
  ...
  list(data = dat_with_na_levels, null_categories = index_for_null_categories, observations_with_more_than_20p_missing_values = observations_with_more_than_20p_missing_values)
} 
...
```{r}
molemmat <- get_data_for_spesific_mca(rbind(matriisi_mca1_missingit, matriisi_mca_missingit), rbind(suplementary_variables1, suplementary_variables))



analyysi_molemmat <- MCA(molemmat$data,
                             ncp=5,
                             quali.sup = which(names(molemmat$data) %in% sup.vars),
                             excl = molemmat$null_categories,
                             ind.sup = c((nrow(vanha$data) + 1):nrow(molemmat$data)), # 2018 individuals are included as support individuals
                             graph = FALSE)
```
