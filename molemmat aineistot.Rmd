...
```{r}
attributes(data1$koulutusaste_4luok) <- NULL
attributes(data1$k1) <- NULL
attributes(data1$k7) <- NULL
attributes(data1$k26a) <- NULL
...
```{r}
data <- haven::read_sav("C:/Users/Ossi/OneDrive/KOULU 2018-2019/Suomalaisen kulttuurin muutos/uusi_aineisto_korjattu_5_3.sav")
...
attributes(data$koulutus4luokkainen) <- NULL
attributes(data$K2) <- NULL
attributes(data$Alue) <- NULL
attributes(data$tulotaso) <- NULL
...
yhteisöpalvelut <- data %>% 
  select("Youtube" = K49_a, "Facebook" = K49_b, "Twitter" = K49_c, "Instagram" = K49_d, "TripAdvisor" = K49_e, "Suomi24" = K49_f,
         "IS tai IL keskustelupalsta" = K49_g, "MV_lehti" = K49_h, "Jokin harrastesivu" = K49_i, "Jokin poliittinen sivu" = K49_j) %>% 
  ...
  mutate(Suomi24 = replace(Suomi24, Suomi24 == 2, 1)) %>% 
  mutate(MV_lehti = replace(MV_lehti, MV_lehti == 2, 1)) %>% 
  funktio_3lk_plus()
names(yhteisöpalvelut)
...
harrastukset <- data %>% 
  select("Puutarhanhoito" = K51_a, "Leipominen" = K51_b, "Käsityöt" = K51_c, "Tekniset työt" = K51_d, "Kalastus" = K51_e, "Metsästys" = K51_f,
         ...
         "Lautapelien pelaaminen" = K51_j, "Pihapelin_pelaaminen"  = K51_k) %>% 
  mutate_each(funs(replace(., . == 3, 0))) %>% 
  mutate_each(funs(replace(., . == 1, 4))) %>% 
  mutate_each(funs(replace(., . == 2, 1))) %>%
  mutate_each(funs(replace(., . == 4, 2))) %>%
  mutate(Metsästys = replace(Metsästys, Metsästys == 2, 1)) %>% 
  mutate(Pihapelin_pelaaminen = replace(Pihapelin_pelaaminen, Pihapelin_pelaaminen == 2, 1)) %>% 
  funktio_3lk_plus()
...
mod_luokitukset <- data.frame(
  mod = unname(unlist(sapply(matriisi_mca_missingit[, !names(matriisi_mca_missingit) %in% sup.vars], function(x) {x <- as.factor(x); levels(x)}))),
  mod_luokitukset
)

mod_luokitukset_uudet_muuttujat <- data.frame(
  mod = unname(unlist(sapply(uudet_muuttujat, function(x) {x <- as.factor(x); levels(x)}))),
