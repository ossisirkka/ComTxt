...
osallistuminen <- data1 %>% 
  select("Museot"= k57b, "Pubit" = k57c, "Karaoke"  = k57d, "Rockkeikat" = k57e, "Oopperakäynti" = k57f, "Bingo" = k57g, "Klassinenkons" = k57h, "Jazzkons" = k57i, "Musikaalit" = k57j, "Teatteri" = k57k, "Galleriat"  = k57l, "Yökerho" = k57m, "Ulkonasyönti" = k57n, "Elokuvat" = k57a) %>%
  mutate_each(funs(replace(.,.==1,2))) %>%
...
taidesuuntaukset <- data1 %>% 
  ...
  mutate(EiMikäänTaide = case_when(
    k45a == 8 & k45b != 8 ~ "EiMikäänTaide+",
    k45a != 8 & k45b == 8 ~ "EiMikäänTaide-", 
    k45a != 8 & k45b != 8 ~ "EiMikäänTaide0"
  )) %>% 
  as.data.frame(.) %>% 
  select(-k45a, -k45b) %>% 
  mutate_all(.,funs(as.factor(.)))
...
```{r}
kirjagenret <- data1 %>% 
  select("Dekkarit" = k39a, "Scifikirjat" = k39b, "Viihde/romant.kirjat" = k39c, "Elämäkerrat" = k39d, "Modernikirjal" = k39e, "Klassinenkirjal" = k39f, "Tietokirjat" = k39g, "Runot" = k39h, "Uskonn.kirjat" = k39i, "Itseapukirjat" = k39j, "Harrastekirjat" = k39k) %>% 
  mutate_each(funs(replace(.,.==1,2))) %>%
  mutate_each(funs(replace(.,.==2,2))) %>%
  mutate_each(funs(replace(.,.==3,1))) %>%
  mutate_each(funs(replace(.,.==6,1))) %>%
...
```{r}
tvsarjat <- data1 %>% 
  select("Kotikatu" = k33_1, "BigBrother" = k33_2, "Sinkkuelämää" = k33_5, "Simpsonit" = k33_7, "A-studio" = k33_10, "Morse" = k33_12, "Miljonääriksi" = k33_15, "SalatutElämät" = k33_19) %>% 
  mutate_each(funs(replace(.,is.na(.),0))) %>% 
  mutate_each(funs(replace(.,.>1,1))) %>% 
  funktio_2lk()
...
#    is.na(K58_a) & is.na(K58_b) & is.na(K58_c) ~ "yleisurheilu0"
  ...
  mutate(TV_Pesäpallo = case_when(
    ...
    K58_c == 1 ~ "TV_Pesäpallo-",
    TRUE ~ "TV_Pesäpallo0"
  )) %>% 
  mutate(TV_Jääkiekko = case_when(
    K58_a == 3 | K58_b == 3 ~ "TV_Jääkiekko+",
    K58_c == 3 ~ "TV_Jääkiekko-",
    TRUE ~ "TV_Jääkiekko0"
  )) %>% 
  mutate(TV_Jalkapallo = case_when(
...
```{r}
osallistuminen <- data %>% 
  select("Museot"= K52_b, "Pubit" = K52_d, "Karaoke"  = K52_e, "Rockkeikat" = K52_f, "Oopperakäynti" = K52_g, "Bingo" = K52_h, "Klassinenkons" = K52_i, "Jazzkons" = K52_j, "Musikaalit" = K52_k, "Teatteri" = K52_l, "Galleriat"  = K52_m, "Yökerho" = K52_n, "Ulkonasyönti" = K52_o, "Elokuvat" = K52_a) %>%
  mutate_each(funs(replace(.,.==1,2))) %>%
  mutate_each(funs(replace(.,.==2,2))) %>%
  mutate_each(funs(replace(.,.==3,2))) %>%
  mutate_each(funs(replace(.,.==4,1))) %>%
...
taidesuuntaukset <- data %>% 
  ...
  mutate(EiMikäänTaide = case_when(
    K36_a == 8 & K36_b != 8 ~ "EiMikäänTaide+",
    K36_a != 8 & K36_b == 8 ~ "EiMikäänTaide-", 
    K36_a != 8 & K36_b != 8 ~ "EiMikäänTaide0"
  )) %>% 
  as.data.frame(.) %>% 
  select(-K36_a, -K36_b) %>% 
  mutate_all(.,funs(as.factor(.)))
...
```{r}
kirjagenret <- data %>% 
  select("Dekkarit" = K31_a, "Scifikirjat" = K31_b, "Viihde/romant.kirjat" = K31_c, "Elämäkerrat" = K31_d, "Modernikirjal" = K31_e, "Klassinenkirjal" = K31_f, "Tietokirjat" = K31_g, "Runot" = K31_h, "Uskonn.kirjat" = K31_i, "Itseapukirjat" = K31_j, "Harrastekirjat" = K31_k) %>% 
  mutate_each(funs(replace(.,.==1,2))) %>%
  mutate_each(funs(replace(.,.==2,2))) %>%
  mutate_each(funs(replace(.,.==3,1))) %>%
  mutate_each(funs(replace(.,.==6,1))) %>%
...
```{r}
tvsarjat <- data %>% 
  select("Kotikatu" = K25_1, "BigBrother" = K25_2, "Sinkkuelämää" = K25_5, "Simpsonit" = K25_7, "A-studio" = K25_10, "Morse" = K25_12, "Miljonääriksi" = K25_15, "SalatutElämät" = K25_19) %>% 
  mutate_each(funs(replace(.,is.na(.),0))) %>% 
  mutate_each(funs(replace(.,.>1,1))) %>% 
  funktio_2lk()
...
save(
  ...
  vanha_indeksöinti,
  uusi_indeksöinti,
  file = "C:/Users/Ossi/OneDrive/KOULU 2018-2019/Suomalaisen kulttuurin muutos/git_files/Suomalaisen-kulttuurin-muutos/data_for_mca.RData"
)
