...
```
```{r,stringtoredoucemodalities}
index_str <- c("Scifielokuvat", "TV_Pesäpallo", "Urheiluohjelmat", "Mäkelä", "Harlin", "Morse", "Poliisisarjat", "Musikaalielokuvat", "TV_Yleisurheilu",
...
vanha <- vanha[,!names(vanha) %in% index_str]
# Create mca-analysis
...
```{r secondAnalysis}
uusi <- uusi[,!names(uusi) %in% index_str]
...
new_variables <- uudet_muuttujat[,names(uudet_muuttujat) %in% c("Helsingin.Sanomat","Taidearvostelut", "Jooga", "Marjastus.tai.sienestys",
 ...
 newdat <- data.frame(complete_uusi_aineisto$completeObs[,!names(complete_uusi_aineisto$completeObs) %in% index_str], new_variables)
...
#    filter(str_sub(rownames(all_coordinates),-1) == sign)
  ...
  ggplot(to_p, aes(x = d1, y = d2, color = vuosi, label = nimi)) +
    ...
    geom_vline(xintercept = 0, colour = "gray70") +
    geom_point(size = 1) +
    geom_line(aes(group = pari),color = "black", size = 0.01, linetype = "dashed", alpha = 0.6) +
...
p1 <- plot_change(1, 25, sign = "+")
p2 <- plot_change(-1, 25, sign = "-")
