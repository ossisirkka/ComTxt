...
kenttä_1 <- newdat_mca$var$contrib[,1]
kenttä_2 <- newdat_mca$var$contrib[,2]

luo_taulukko <- function(otsikko){
  # Funktio osuuksien laskemiseen -------------------------------------------
  
  kokonaiskontribuutio_vanha_aineisto <- function(kenttä1, kenttä2, kenttä) {
    ensimmäinen <- sum(kenttä1) / sum(kenttä_1)
    toinen <- sum(kenttä2) / sum(kenttä_2)
    out <- c(kenttä, round(ensimmäinen, 3),round(toinen, 3))
    return(out)
  }
  ...
  mod_musiikki_1 <- kenttä_1[c(
    43:52,   # Festarit pl. suurajot
    88:96,   # karaoke, rockkeikka, oopperakäynti
    100:108, # klassinen konsertti, jazzkonsertti, musikaalit
    157:207, # artistit, genret
    350:352, # levyt
    413:414  # cheeck
  )]
  mod_musiikki_2 <- kenttä_2[c(
    43:52,   # Festarit pl. suurajot
    88:96,   # karaoke, rockkeikka, oopperakäynti
    100:108, # klassinen konsertti, jazzkonsertti, musikaalit
    157:207, # artistit, genret
  ...
  mod_kirjallisuus_1 <- kenttä_1[c(
    208:271, # kirjailijat, genret ja lehdet
    356:358, # kirjoja kotona
    365:367, # lukeminen
    109:111, # Teatteri
    407:412  # Helsingin sanomat, kauppalehti
  )]
  
  mod_kirjallisuus_2 <- kenttä_2[c(
    208:271, # kirjailijat, genret ja lehdet
    356:358, # kirjoja kotona
    365:367, # lukeminen
    109:111, # Teatteri
  ...
  mod_kuvataide_1 <- kenttä_1[c(
    124:156, # Taiteilijat, taidesuuntaukset
    359:361, # tauluja kotona
    112:114, # galleriat
    82:84,   # museot
    415:418  # taidearvostelut ja cheeck
  )]
  mod_kuvataide_2 <- kenttä_2[c(
    124:156, # Taiteilijat, taidesuuntaukset
    359:361, # tauluja kotona
    112:114, # galleriat
    82:84,   # museot
  ...
  mod_tv_elokuvat_1 <- kenttä_1[c(
    121:123, # elokuvissa käynti
    272:331, # ohjaajat, elokuvagenret, tv-ohjelmat, tv-ohjelmagenret
    353:355, # dvd kotona
    362:364  # tv:n katselu
  )]
  mod_tv_elokuvat_2 <- kenttä_2[c(
    121:123, # elokuvissa käynti
    272:331, # ohjaajat, elokuvagenret, tv-ohjelmat, tv-ohjelmagenret
    353:355, # dvd kotona
    362:364  # tv:n katselu
  ...
  mod_urheilu_muut_1 <- kenttä_1[c(
    1:42,   # tv-liikunta, harrastukset
    53:54,  # suurajot
    97:99,  # Bingo
    115:117,# yökerho
    332:343,# liikunta, lävistys, tatuointi, muodikas, solarium, merkkitietoinen
    427:434 # jooga, kehonrakennus, edullinen pukeutuminen
  )]
  mod_urheilu_muut_2 <- kenttä_2[c(
    1:42,   # tv-liikunta, harrastukset
    53:54,  # suurajot
    97:99,  # Bingo
    115:117,# yökerho
  ...
  mod_ruoka_1 <- kenttä_1[c(
    55:81,   # ruoista pitäminen
    118:120, # ulkona syöminen
    344:349, # ruokajuoma
    85:87    # Pubit
  )]
  mod_ruoka_2 <- kenttä_2[c(
    55:81,   # ruoista pitäminen
    118:120, # ulkona syöminen
    344:349, # ruokajuoma
    85:87    # Pubit
  ...
  #  # Pelaaminen
  pelaaminen_1 <- kenttä_1[c(
    368:373, # peligenret
    404:406  # pelaaminen tunteina
  )]
  pelaaminen_2 <- kenttä_2[c(
    368:373, # peligenret
    404:406  # pelaaminen tunteina
  )]
  ...
  # # Arkiset
  arkiset_1 <- kenttä_1[c(
    392:403 # arkiset puuhastelut
  )]
  arkiset_2 <- kenttä_2[c(
    392:403 # arkiset puuhastelut
  )]
  
  # # Lomailu
  lomailu_1 <- kenttä_1[c(
    419:426
  )]
  lomailu_2 <- kenttä_2[c(
    419:426
  )]
  
  # Taulukko tuloksista -----------------------------------------------------
  
  tulokset_vanha_aineisto <- data.frame(rbind(
    kokonaiskontribuutio_vanha_aineisto(mod_musiikki_1, mod_musiikki_2, "Musiikki"),
    kokonaiskontribuutio_vanha_aineisto(mod_kirjallisuus_1, mod_kirjallisuus_2, "Kirjallisuus ja lukeminen"),
    kokonaiskontribuutio_vanha_aineisto(mod_kuvataide_1, mod_kuvataide_2, "Kuvataide"),
    kokonaiskontribuutio_vanha_aineisto(mod_tv_elokuvat_1, mod_tv_elokuvat_2, "Tv ja elokuvat"),
    kokonaiskontribuutio_vanha_aineisto(mod_urheilu_muut_1, mod_urheilu_muut_2, "Urheilu, pukeutuminen, ruumiillisuus"),
    kokonaiskontribuutio_vanha_aineisto(mod_ruoka_1, mod_ruoka_2, "Ruoka ja syöminen"),
    kokonaiskontribuutio_vanha_aineisto(pelaaminen_1, pelaaminen_2, "Pelaaminen"),
    kokonaiskontribuutio_vanha_aineisto(online_1, online_2, "Online"),
    kokonaiskontribuutio_vanha_aineisto(arkiset_1, arkiset_2, "Arkiset"),
    kokonaiskontribuutio_vanha_aineisto(lomailu_1, lomailu_2, "Lomailu")
  ), stringsAsFactors = FALSE)
  
  
  names(tulokset_vanha_aineisto) <- c("Kenttä", "Kontribuutio ensimmäiselle ulottuvuudelle", "Kontribuutio toiselle ulottuvuudelle")
  
  Modaliteetit <- c(length(mod_musiikki_1),
                    length(mod_kirjallisuus_1),
                    length(mod_kuvataide_1),
                    length(mod_tv_elokuvat_1),
                    length(mod_urheilu_muut_1),
                    length(mod_ruoka_1),
                    length(pelaaminen_1),
                    length(online_1),
                    length(arkiset_1),
  ...
  tulokset_vanha_aineisto <- tibble::add_column(tulokset_vanha_aineisto, Modaliteetit, .after = "Kenttä")

  Osuus <- c(length(mod_musiikki_1) / 434,
             length(mod_kirjallisuus_1) / 434,
             length(mod_kuvataide_1) /  434,
             length(mod_tv_elokuvat_1) / 434,
             length(mod_urheilu_muut_1) / 434,
             length(mod_ruoka_1) / 434,
             length(pelaaminen_1) / 434,
             length(online_1) / 434,
             length(arkiset_2) / 434,
             length(lomailu_1) / 434)
            
  Osuus <- round(Osuus, 2)
  
  tulokset_vanha_aineisto <- tibble::add_column(tulokset_vanha_aineisto, Osuus, .after = "Modaliteetit")
  
  kontribuutio_molemmille_ulottuvuuksille <- round(c(
    sum(mod_musiikki_1, mod_musiikki_2) / sum(kenttä_1, kenttä_2),
    sum(mod_kirjallisuus_1, mod_kirjallisuus_2) / sum(kenttä_1, kenttä_2),
    sum(mod_kuvataide_1, mod_kuvataide_2) / sum(kenttä_1, kenttä_2),
    sum(mod_tv_elokuvat_1, mod_tv_elokuvat_2) / sum(kenttä_1, kenttä_2),
    sum(mod_urheilu_muut_1, mod_urheilu_muut_2) / sum(kenttä_1, kenttä_2),
    sum(mod_ruoka_1, mod_ruoka_2) / sum(kenttä_1, kenttä_2),
    sum(pelaaminen_1, pelaaminen_2) / sum(kenttä_1, kenttä_2),
    sum(online_1, online_2) / sum(kenttä_1, kenttä_2),
    sum(arkiset_1, arkiset_2) / sum(kenttä_1, kenttä_2),
    sum(lomailu_1, lomailu_2) / sum(kenttä_1, kenttä_2)), 2)
  
  tulokset_vanha_aineisto[,6] <- kontribuutio_molemmille_ulottuvuuksille
  
  tulokset_vanha_aineisto[11,] <- cbind(c("Yhteensä", apply(apply(tulokset_vanha_aineisto[,2:6], 2, as.numeric), 2, function(x) sum(x))))
  
  names(tulokset_vanha_aineisto)[6] <- "Kontribuutio yhteensä molemmille akseleille"
  
  kenttien_koot <- tulokset_vanha_aineisto[,1:3]
  tulokset_vanha_aineisto <- tulokset_vanha_aineisto[,-c(2:3)]
  
  return(list(kenttien_koot, tulokset_vanha_aineisto))
}

create_kable(luo_taulukko()[[1]],c(2:3),("Taulukko 2: Kenttien koot täydennetyllä listalla muuttujia"))
...
Explanatory analysis of third dimension did not show any consistent structure on the axis. The first and second dimensions are presented in figure 3.

```{r Figure3, fig.cap="First and second dimensions of space of lifestyles in Finland 2018", fig.pos = "c", out.width = "100%"}
...
Similar items are near the origo of the space of lifestyles in 2018 as they were in 2007. Cultural items such as liking pizza or reading Pirkka-magazine, which is the most popular magazine in Finland. 

### Modalities from 2018 data projected to the map constructed using 2007 data


Using these distances, we can determine which modalities have experienced greatest changes in their location in space of lifestyles during eleven years. 20 modalities with highest and smallest difference in their location is presented in figure 5..

```{r figure5, fig.cap="Items with highest and smallest difference in their location in space of lifestyles during 2007-2018 in Finland.", out.width="100%"}
...
```{r figure6, fig.cap="Cloud of individuals of 2018 as supplementary individuals in cloud of active individuals of 2007", out.width="100%"}
...
To cluster our data, we use hierarchical clustering on principal components from FactoMineR r-package (Husson et al. 2010). Results from cluster analysis on axes obtained by multiple correspondence analysis are presented in figures 8 and 9. To determine the most proper number of clusters, we use recommended number of clusters by HCPC-function from FactoMineR-package, which seems to be 3.


```{r Figure8, fig.cap="Clusters in spaces of lifestyles in 2007 and 2018.", out.width="100%"}
...
Previous analyses were conducted using set of cultural indicators, that were available in both 2007 and 2018 data. Next, we will conduct multiple correspondence analysis to 2018 data using set of cultural indicators that yields the most comprehensive space of lifestyles using our data. (note: Proportion of variance explained could be increased to ~78 % by dismissing "central"/not-separating cultural indicators (mökki, risteily, cheek). Maybe not worth it, when interprepting these variables is interesting?) New variables include both online and offline indicators, such as reading Helsingin Sanomat newspaper, body building, liking art of Banksy, using social media service Twitter, reading blogs online or watching videos from Youtube. Total number of new variables included is `r ncol(new_variables)`. Using these variables as active variables, the axes 1 and 2 of our more comprehensive space of lifestyles is presented in figure 10. These axes are presented in two-dimensional plane in figure 11, with newly included variables highlighted.
...
```{r, figure11, fig.cap="Space of lifestyles in 2018 using comprehensive set of cultural indicators.", out.width="100%"}
