...
tables_df <- add_column(tables_df,
           Class = c(mod_luokitukset, rep(" ", mod_määrä(uusi[,names(uusi) %in% sup.vars]))),
...
new_vars_tables <- lapply(newdat$data[, which(names(newdat$data) == "Toimintapelit"):ncol(newdat$data)], table)
