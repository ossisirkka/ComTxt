...
```{r,indexstring_to_reduce_modalities}
load("index_for_removing_mod_classifiers.RData")
mod_luokitukset <- mod_luokitukset[!index_for_removing_mod_classifiers]
```
...
new_variables <- uudet_muuttujat[,names(uudet_muuttujat) %in% c("Helsingin.Sanomat","Taidearvostelut", "Jooga", "Marjastus.tai.sienestys",
 ...
 newdat <-get_data_for_spesific_mca(cbind(matriisi_mca_missingit, new_variables), suplementary_variables)
...
The total number of observations in 2007 data is `r nrow(vanha$data)`. `r 1388 - nrow(vanha$data)` observations were dropped from the sample due to high fraction of missing data. In 2018 data, there are `r nrow(uusi$data)` observations, and `r 1425 - nrow(uusi$data)` observations were deleted. For the remaining observations, Multiple imputation method suggested by Josse & Husson (2016) were employed. In the first two steps of analysis, there are `r round(nrow(vanha$data) / (ncol(vanha$data)-length(sup.vars)),2)` and `r round(nrow(uusi$data) / (ncol(uusi$data)-length(sup.vars)),2)`  individuals for active category. In the literature, one suggested guideline to minimum number of observations per active category is 20 (Di Franco 2015, 1305). However, author do not present any mathematical justification or citations for this precept. Despite of low number of observations per active category, we proceed to conduct our analysis with high total number of active categories.   
...
As a result of high number of variables included in our analysis, conventional presentation of all modalitites with higher than mean contribution to the axis is not possible. In figure 1, 20 items in each axis with highest and lowest coordinates are presented. 

```{r Figure1, fig.cap="First and second dimension in 2007 analysis", fig.pos = "c", out.width = "100%"}
library(ggrepel)
...
Eigenvalues, percentages of explained variance and Benzecri's modified explanation rates of first 4 axes are presented in table 3. 
...
Compared to analysis conducted using 2007 data, first dimension is almost equally strong in explaining variance of the data. Examining second dimension, it seems that there is a drop in explanatory power compared to analysis conducted using 2007 data, and this will be addressed later in the analysis. Together first and second axis account for `r round(sum(korjatut_selitysasteet(analyysi_uusi)[1], korjatut_selitysasteet(analyysi_uusi)[2]),2)` percent of variance according to Benzecri's modified rates. This is slightly below recommended 80 % (Hjellbrekke 2018). Still, only first and second dimension are interpreted. The contributions of fields included in the analysis are presented in table 4.
...
```{r, Figure6, fig.cap="Clouds of individuals in 2018 according to socio-ecominc variables", out.width="100%"}
...
plottaa_uudet_muuttujat(matriisi_mca_missingit, new_variables, suplementary_variables, 0.2)
```
...
df_temp <- data.frame(complete_uusi_aineisto$completeObs[uusi_indeksöinti,!names(complete_uusi_aineisto$completeObs) %in% sup.vars],
                      data.frame(new_variables)[uusi_indeksöinti,],
                      complete_uusi_aineisto$completeObs[uusi_indeksöinti, names(complete_uusi_aineisto$completeObs) %in% sup.vars]
                      )


df_temp_comp <- missMDA::imputeMCA(df_temp,ncp=3)
mca_temp <- MCA(df_temp_comp$completeObs,
                quali.sup = which(names(df_temp_comp$completeObs) %in% sup.vars),
                graph = FALSE
)

plot_sup_vars(mca_temp, "2018")
```
...
# Conclusions
  ...
  * Two dimensions are sufficient to represent space of lifestyles in Finland, both 2007 and 2018
    ...
    * Considering individual items used to construct the space of lifestyles in 2007 and 2018, some cultural indicators experienced notable change in their location in the space. 
...
new_tables_df <- add_column(new_tables_df[str_detect(new_tables_df$Label, "NA", negate = TRUE),], Class = newmods, .after = "Label")
