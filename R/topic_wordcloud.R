...
topic_wordcloud <- function(par_row = 3, par_col = 3, n_topic =7,maximum_n = 200){
  ...
  for (i in 1:as.numeric(n_topic)) {
    mallet_words_list[[i]] <- mallet.top.words(mallet_df, topic.words[i,], m_w)
  }
  topic_mallet_list <- mallet_words_list

  for(i in 1:length(topic_mallet_list)){
    df_words <- as.data.frame(topic_mallet_list[i])
    wordcloud(df_words$words, df_words$weights, scale = c(3,.2),
