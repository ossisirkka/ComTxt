...
homogenity_test_for_pc_axis <- function(var = NULL, axis = 1){
  ...
  for(mod in mods) {
    ...
    n_tilda <- 1/(
      1 / length(molemmat[1:n_2007, var] == mod) +
        1 / length(molemmat[(n_2007+1) : n_2018, var] == mod)    
    )
...
homogenity_test_for_plane <- function(var = NULL){
  ...
  for(mod in mods) {
    ...
    lambda_1 <- analyysi_molemmat$eig[1]
    lambda_2 <- analyysi_molemmat$eig[2]
...
res_df_axis1 <- do.call(rbind, lapply(names(molemmat[,!names(molemmat) %in% sup.vars]), homogenity_test_for_pc_axis))
res_df_axis2 <- do.call(rbind, lapply(names(molemmat[,!names(molemmat) %in% sup.vars]), homogenity_test_for_pc_axis, axis = 2))
res_df_plane <- do.call(rbind, lapply(names(molemmat[,!names(molemmat) %in% sup.vars]), homogenity_test_for_plane))
```
...
Next, we examine the change in terms of movement in two main axis between 2007 and 2018 individuals in distinct sociodemographic groups. This shift is analyzed by comparing mean points of individuals that chose particular sociodemographic modality in 2007 and 2018. This analysis is presented in table  `r tablenum <- tablenum + 1 ; tablenum`.
