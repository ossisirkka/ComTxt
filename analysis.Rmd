...
```{r global_options_local, include=FALSE}
...
Our analysis will be conducted in three separate steps. First, we create space of lifestyles in Finland in 2007 using `r ncol(molemmat$data[,!names(molemmat$data) %in% sup.vars])` active variables and `r length(sup.vars)` supplementary variables. After careful examination of space of lifestyles in 2007, we proceed to step 2 and conduct analysis using 2018 data with identical indicators. To examine change that has happened during 11 years, from 2007 to 2018, we use the respondents from 2018 as supplementary individuals in space constructed using individuals from 2007. In third step, we introduce new cultural practices to space of lifestyles in 2018. These new practices consist both offline and online cultural indicators, such as hunting, gardening and art by Banksy, and using the internet or playing games.   

## Space of lifestyles in 2007 and 2018 with identical set of indicators

We start our analysis by creating the spaces of lifestyles using data from 2007 and 2018. Table `r tablenum <- tablenum + 1 ; tablenum ` and `r tablenum <- tablenum + 1 ; tablenum ` shows four first dimensions produced by MCA by respective year. 

```{r Table3 }
create_kable <- function(table, cols = ncol(table) , otsikko, formaatti = "html"){
  return(knitr::kable(table, caption = otsikko, format = formaatti) %>%
...
As a result of high number of variables included in our analysis, conventional presentation of all modalitites with higher than mean contribution to the axis is not possible. In figure `r fignum <- 1 ; fignum `, 10 items in each axis for both years with highest and lowest coordinates are presented. 
...
Next, we will analyze the change in the space of lifestyles from 2007 to 2018. To examine change that has happened, we compare locations of individual lifestyle indicators in both first axes, and in the space of lifestyles as a two dimensional plane. In figure  `r fignum <- fignum + 1 ; fignum`, distributions of all changes aggregated by fields are presented. 

```{r, calculate_sup_vars}
...
calculate_sup_vars <- function(dat, sarake){
  ...
  df <- data.frame(dat$call$X[(nrow(vanha$data) + 1):nrow(dat$call$X),!names(dat$call$X) %in% index_str], dim1, dim2, weigth) # dataframe, where is imputated survey data and our coordinates
  
  temp <- split(df, df[,sarake]) # Split the data to as many sets as there are categories in the variable
  temp <- temp[str_detect(names(temp), "NA", negate = TRUE)] # Delete NA-split from the data; this is not part of the analysis
...
homogenity_test_for_pc_axis <- function(var = NULL, axis = 1){
  
  mods <- levels(molemmat$data[,var])[str_detect(levels(molemmat$data[,var]), "NA", negate = TRUE)]
  out <- data.frame(matrix(ncol = 3))
  names(out) <- c("mod", paste0("Z_stat_", axis), paste0("pval_", axis))
  lambda <- analyysi_molemmat$eig[axis,1]
  total_n <- nrow(molemmat$data)
  
  for(mod in mods) {
    n_2007 <- nrow(analyysi_molemmat$ind$coord)
    n_2018 <- nrow(analyysi_molemmat$ind.sup$coord)
    
    n_tilda <- 1/(
      1 / length(molemmat$data[1:n_2007, var] == mod) +
        1 / length(molemmat$data[(n_2007+1) : n_2018, var] == mod)    
    )
    
    V <- (total_n / (total_n - 1)) * (lambda / n_tilda) # Variance of permutation distribution
    
    d_diacritic <- all_coordinates[mod, paste0("dim", axis, "_07")] - all_coordinates[mod, paste0("dim", axis, "_18")]
    
    Z_stat <- d_diacritic / sqrt(V)
    p_Z <- dnorm(Z_stat, mean = 0, sd = 1)
    out[which(mods == mod),] <- cbind(mod = mod, Z = Z_stat, pval = p_Z)
...
homogenity_test_for_plane <- function(var = NULL){
  
  mods <- levels(molemmat$data[,var])
  out <- data.frame(matrix(ncol = 3))
  names(out) <- c("mod", "chisq", "pval")
  
  for(mod in mods) {
    n_2007 <- nrow(analyysi_molemmat$ind$coord)
    n_2018 <- nrow(analyysi_molemmat$ind.sup$coord)
    
    lambda_1 <- analyysi_molemmat$eig[1,1]
    lambda_2 <- analyysi_molemmat$eig[2,1]
    
    d_hat_1 <- all_coordinates[mod, "dim1_07"] - all_coordinates[mod, "dim1_18"]
    d_hat_2 <- all_coordinates[mod, "dim2_07"] - all_coordinates[mod, "dim2_18"]
    
    D_squared <- (d_hat_1^2 / lambda_1) + (d_hat_2^2 / lambda_2) 
    ...
    chi_squared <- n_tilda * ((n_2007 + n_2018 -1) / (n_2007 + n_2018)) * D_squared
    p_chi <- dchisq(chi_squared, 2)
    out[which(mods == mod),] <- cbind(mod = mod, chisq = chi_squared, pval = p_chi)
...
res_df_axis1 <- do.call(rbind, lapply(names(molemmat$data[,!names(molemmat$data) %in% sup.vars]), homogenity_test_for_pc_axis))
res_df_axis2 <- do.call(rbind, lapply(names(molemmat$data[,!names(molemmat$data) %in% sup.vars]), homogenity_test_for_pc_axis, axis = 2))
res_df_plane <- do.call(rbind, lapply(names(molemmat$data[,!names(molemmat$data) %in% sup.vars]), homogenity_test_for_plane))
```


```{r, fig.cap="Distributions of difference in locations of lifestyle indicators in 2007 and 2018 in first and second axis and in the two-dimensional plane. Outliers in each fields are labeled.", out.width="100%"}





tmpdat <- all_coordinates %>% 
  add_rownames(var = "mod") %>% 
  inner_join(res_df_axis1, by = "mod") %>% 
  mutate(
    dist_ax1 = abs(dim1_07 - dim1_18),
    dist_ax2 = abs(dim2_07 - dim2_18),
    kenttä = mod_luokitukset) 
...
p1 <- all_coordinates %>% 
  ...
  mutate(
    ...
    dist_ax2 = abs(dim2_07 - dim2_18),
    kenttä = mod_luokitukset) %>% 
  ggplot(aes(x = kenttä, y = dist_ax1, color = kenttä)) +
  geom_boxplot() +
  guides(colour = FALSE) + 
  ggtitle("Modaliteettien sijaintien välinen ero vuosien 2007 ja 2018 välilä") +
  ylab("Ensimmäinen ulottuvuus") +
  xlab("") +
  ylim(0, 1) +
  coord_flip() +
  theme_minimal() + 
...
names(out) <- levels(factor(mod_luokitukset))
tidyout <- purrr::map_df(out, tibble::as_tibble, .id = "kenttä")
labels_dat <- tmpdat %>% 
  mutate(value = dist_ax1) %>% 
  inner_join(tidyout, by = "value") %>% 
  select(kenttä = kenttä.y, value, mod) %>% 
  arrange(kenttä)
p1 <- p1 + geom_text_repel(data = labels_dat, aes(kenttä, value, label = mod), 
...
p2 <- all_coordinates %>% 
  ...
  mutate(
    ...
    dist_ax2 = abs(dim2_07 - dim2_18),
    kenttä = mod_luokitukset) %>% 
  ggplot(aes(x = kenttä, y = dist_ax2, color = kenttä)) +
  geom_boxplot() +
  guides(colour = FALSE) + 
  ylab("Toinen ulottuvuus") +
  xlab("") +
  ylim(0, 1) +
  coord_flip() +
  theme_minimal() +
...
out <- ggplot_build(p2)[["data"]][[1]][["outliers"]]
names(out) <- levels(factor(mod_luokitukset))
tidyout <- purrr::map_df(out, tibble::as_tibble, .id = "kenttä")
labels_dat <- tmpdat %>% 
  mutate(value = dist_ax2) %>% 
  inner_join(tidyout, by = "value") %>% 
  select(kenttä = kenttä.y, value, mod) %>% 
  arrange(kenttä)
p2 <- p2 + geom_text_repel(data = labels_dat, aes(kenttä, value, label = mod), 
                           hjust = 1, vjust = 1, size = 5)
...
p3 <- all_coordinates %>% 
  ...
  mutate(
    ...
    dist_ax2 = abs(dim2_07 - dim2_18),
    kenttä = mod_luokitukset) %>% 
  ggplot(aes(x = kenttä, y = euc_dist, color = kenttä)) +
  geom_boxplot() +
  guides(colour = FALSE) +
  ylab("Kaksiulotteisella kartalla, euklidinen etäisyys") +
  xlab("") +
  ylim(0, 1) + 
  coord_flip() + 
  theme_minimal() +
...
out <- ggplot_build(p3)[["data"]][[1]][["outliers"]]
names(out) <- levels(factor(mod_luokitukset))
tidyout <- purrr::map_df(out, tibble::as_tibble, .id = "kenttä")
labels_dat <- tmpdat %>% 
  mutate(value = euc_dist) %>% 
  inner_join(tidyout, by = "value") %>% 
  select(kenttä = kenttä.y, value, mod) %>% 
  arrange(kenttä)
p3 <- p3 + geom_text_repel(data = labels_dat, aes(kenttä, value, label = mod), 
                           hjust = 1, vjust = 1, size = 5)
...
Figure `r fignum` shows that changes during eleven years were more significant in second than in first axis.  Considering the first axis, most stable fields of lifestyles was arts and urheilu, pukeutuminen ja ruumiillisuus, and most dynamic fields were tv and movies and food. Outliers, marked as dot and label, reveal some of the most stable and most dynamic lifestyle indicators. In the second axis, there are more variance and on average higher differences in the locations, which indicates lifestyle indicators have shifted in the dimension of modernity in more diverse manner during 11 years, comparing to dimension of legitimacy. Third part of figure `r fignum` presents euclidian distance of modalities in 2007 and 2018 in two-dimensional plane, and it reflects results from first and second axis. To show the modalities with biggest differences in their locations in space of lifestyles and to examine the direction of change, we present figure  `r fignum <- fignum + 1 ; fignum`. In Appendix 3, all statistically significant (p < 0.01) changes of modalities in axis 1, axis 2, or in the plane composed of those axes are presented. 
...
inds <- rbind(
      data.frame(dim1 = analyysi_molemmat$ind$coord[,1],
...
Clouds composed of individuals from 2007 and 2018 seem to be quite similar in their location. Individuals from 2018 are slightly shifted to more legitimate and modern side of the space of lifestyles, and they are slightly more concentrated towards center of the space. In table `r tablenum <- tablenum + 1 ; tablenum`, means and variances of locations in these two dimensions are presented.
...
In the first dimension, shift of individuals is towards legitimate side in space of lifestyles compared to 2007 (p = `r table3[1,4]`). In second dimension, 2018 individuals were more modern than individuals in 2007 (p < `r table3[2,4]`). Variance in both dimensions were higher among 2007 than 2018 individuals. Shift towards modern side of the space can be partly explained with cohort effect; the most traditional group in 2007 was not present in 2018 data, and oldest cohort in 2018 data was did not prefer as traditional cultural indicators as previous cohort. It is also notable that even individuals in 2018 were more modern than invidivuals in 2007, there are no individuals in 2018 that are more modern than most modern individuals in 2007. This might be explained by identical set of indicators, picked before first survey in 2007. Items that were considered most modern in 2007 are still most modern in 2018, selitä paremmin.

Next, we examine the change in terms of movement in two main axis between 2007 and 2018 individuals in distinct sociodemographic groups. This shift is analyzed by comparing mean points of individuals that chose particular socio demographic modality in 2007 and 2018. This analysis is presented in table  `r tablenum <- tablenum + 1 ; tablenum` and visualized in figure `r fignum <- fignum + 1; fignum`.

```{r}
plot_inds_sup_vars <- function(taustamuuttuja){
  ...
  coords_and_sup_vars <-coords_and_sup_vars %>% 
    group_by(.data[[taustamuuttuja]],aineisto) %>% 
    summarise(Dim.1 = mean(Dim.1),
              Dim.2 = mean(Dim.2) )%>% 
  filter(str_detect(.data[[taustamuuttuja]], "NA", negate = TRUE))
#  return(coords_and_sup_vars)
  
  coords_and_sup_vars$pari <- rep(seq_len(nrow(unique(coords_and_sup_vars[,1]))), each = 2)
  var <- rlang::enquo(taustamuuttuja)
  
  limx <- c(-0.4, 0.4)
  limy <- c(-0.4, 0.4)
  bre <- round(seq(-1.6, 2, 0.2),2)
  ggplot(coords_and_sup_vars, aes(x = Dim.1, y = Dim.2, label = interaction(.data[[taustamuuttuja]], aineisto),
                                  color = .data[[taustamuuttuja]])) +
    geom_point(size = 1) + 
...
p1 <- plot_inds_sup_vars("ikä")
p2 <- plot_inds_sup_vars("koulutus")
p3 <- plot_inds_sup_vars("ammattiluokka")
p4 <- plot_inds_sup_vars("alue")
p5 <- plot_inds_sup_vars("tulotaso")
p6 <- plot_inds_sup_vars("sukupuoli")
...
get_mean_and_test_table2 <- function(muuttuja, dimensio = 1){
  ...
  tmp <- split(coords_and_sup_vars, coords_and_sup_vars[[muuttuja]])
  tmp <- tmp[str_detect(names(tmp), "NA", negate = TRUE)]
  tmp <- do.call(rbind, tmp)
  coords_and_sup_vars <- tmp
  coords_and_sup_vars[[muuttuja]] <- factor(coords_and_sup_vars[[muuttuja]])
...
We can see from table `r tablenum` that two youngest cohorts occupy stable location in the space from 2007 to 2018. For the rest of the cohorts, all of them were more modern in 2018 than in 2007, which might be explained by stability of cultural practices in individuals. Despite of aging, lifestyle of the individual might remain more or less stable, which is indicated by the fact that location of cohorts (excluding two youngest ones) in 2018 reflects locations of the same cohorts in 2007, so, for example cohort of 55-64 year old in 2018 occupies the same location of 45-54 in 2007. With homogeneity tests for principal plane, we find out that change in locations of the cohorts during eleven years was not significant, as presented in table `r tablenum <- tablenum + 1; tablenum`. (liitän nyt kuvankin tähän...)
...
coords_and_sup_vars <-coords_and_sup_vars %>% 
  ...
  summarise(Dim.1 = mean(Dim.1),
            Dim.2 = mean(Dim.2)) %>% 
  filter(str_detect(ikä, "NA", negate = TRUE))

coords_and_sup_vars$pari <- (c(1, 7 , 2, 1 , 3, 2 , 4, 3 , 5, 4 , 8, 5 ))
...
homogenity_test_for_plane_sup_vars <- function(cohort1, cohort2){
    ...
    d_hat_1 <- mean(coords_and_sup_vars[coords_and_sup_vars$aineisto_ikä == cohort1, "Dim.1"]) - mean(coords_and_sup_vars[coords_and_sup_vars$aineisto_ikä == cohort2, "Dim.1"])
    d_hat_2 <- mean(coords_and_sup_vars[coords_and_sup_vars$aineisto_ikä == cohort1, "Dim.2"]) - mean(coords_and_sup_vars[coords_and_sup_vars$aineisto_ikä == cohort2, "Dim.2"])
    
    D_squared <- (d_hat_1^2 / lambda_1) + (d_hat_2^2 / lambda_2) 
    
    n_tilda <- 1/(
      1 / length(coords_and_sup_vars[1:n_2007, "aineisto_ikä"] == cohort1) +
        1 / length(coords_and_sup_vars[(n_2007+1) : n_2018, "aineisto_ikä"] == cohort2)    
    )
    
    chi_squared <- n_tilda * ((n_2007 + n_2018 -1) / (n_2007 + n_2018)) * D_squared
    p_chi <- dchisq(chi_squared, 2)
    cbind(cohort_11 = cohort1, cohort_2 = cohort2, chisq = chi_squared, pval = p_chi)
    #return(list(d_hat_1, d_hat_2, D_squared, n_tilda ))
}
...
coords_and_sup_vars <- data.frame(rbind(analyysi_molemmat$ind$coord[,1:2], analyysi_molemmat$ind.sup$coord[,1:2]),
                                  ...
                                  aineisto = c(rep("2007", nrow(analyysi_molemmat$ind$coord)),
                                               rep("2018", nrow(analyysi_molemmat$ind.sup$coord))))
coords_and_sup_vars <- coords_and_sup_vars  %>% 
  select(Dim.1, Dim.2, ikä, aineisto) %>% 
  mutate(aineisto_ikä = interaction(ikä, aineisto)) 


res <- rbind(
  homogenity_test_for_plane_sup_vars(cohort1 = "18-24.2007", cohort2 = "25-34.2018"),
  homogenity_test_for_plane_sup_vars(cohort1 = "25-34.2007", cohort2 = "35-44.2018"),
  homogenity_test_for_plane_sup_vars(cohort1 = "35-44.2007", cohort2 = "45-54.2018"),
  homogenity_test_for_plane_sup_vars(cohort1 = "45-54.2007", cohort2 = "55-64.2018"),
  homogenity_test_for_plane_sup_vars(cohort1 = "55-64.2007", cohort2 = "65-74.2018")
)
res 
```
...
To cluster our data, we use hierarchical clustering on principal components from FactoMineR r-package (Husson et al. 2010). Results from cluster analysis on axes obtained by multiple correspondence analysis are presented in figures `r fignum <- fignum + 1 ; fignum` and  `r fignum <- fignum + 1 ; fignum`. To determine the most proper number of clusters, we use recommended number of clusters by HCPC-function from FactoMineR-package, which seems to be 3.
...
As can be seen from figure `r fignum`, the first and second dimension remained the same as in previous spaces, despite adding new cultural indicators. Introducing our new set of variables to our analysis yielded higher proportion of explained variance (24 % -> 27.5 %) in traditional-modern -axis, but lowered this proportion in first, illegitimate-legitimate -axis (52 % -> 48.4). In total, these two dimensions explain 76 % percent of the variance in our data. The increased explanation power of second axis is somewhat expected: some of the new cultural indicators, such as gaming and social media indicators, were known to be favored by younger population. As previously, we do not interpret third dimensions.  
...
In figure `r fignum <- fignum + 1 ; fignum`, supplementary variables for analysis with complete set of indicators are presented.
...
```{r}
new_whole <- find_most_important_categories_for_clusters(clust_2018_whole, n = 3, sign = "+", vuosi = "2007")
new_whole
```
